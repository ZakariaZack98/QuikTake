...
const NotePopUp = ({ noteData, onNoteClose }) => {
  ...
  const handleFileChange = async (event) => {
    ...
    if (files) {
      ...
      const updatedPhotoUrls = photoUrls.concat(secureUrls);
  ...
  return (
    <div className="absolute top-0 left-0 w-screen h-screen flex justify-center items-center bg-[rgba(0,0,0,0.69)] z-50">
      <span
        className="close absolute right-5 top-5 text-white text-4xl cursor-pointer"
        onClick={() => onNoteClose(false)}>
        <IoClose />
      </span>
